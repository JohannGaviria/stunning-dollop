...
|             | `APP_DESCRIPTION` | Detailed description of the application                           | `A web platform that allows users to...`  |
...
|             | `BACKEND_HOST`    | Host address for the backend server                               | `localhost`                               |
|             | `BACKEND_WORKERS` | Number of worker processes for handling requests                  | `4`                                       |
|             | `ACME_EMAIL`      | Email for Let's Encrypt SSL certificate notifications             | `admin@yourdomain.com`                    |
|             | `TRAEFIK_AUTH`    | Basic authentication for Traefik dashboard (user:hashed_password) | `admin:$$apr1$$hashed_password`           |
